...
A surber sampler was used to collect invertebrates from stream beds.

```{r}
pacman::p_load("vembedr")
embed_youtube("https://www.youtube.com/watch?v=BNrc9YkPpfA",  ratio="16by9")%>%
  use_bs_responsive()
```
...
```{r}
df.read <- read.csv(file="https://ftp.maps.canada.ca/pub//nrcan_rncan/Forests_Foret/TLW/TLW_invertebrateDensity.csv") # load Turkey Lakes invertebrate density data from the website
colnames(df.read) # take a look at the columns
```
...
-   A column for each of the species with their densities (the density is measured as .....)
...
df.count <- 
  ...
  tidyr::pivot_longer( 
    "Aeshna":"Trichoptera", # Select the columns we want in long format (i.e. the columns with invertebrate species)
    names_to = "Species", # Name the column that we are creating for the invertebrate species
    values_to = "Density" ) %>% # Name the column that we want the density values to do to
  mutate(
    Count = Density * 0.33, # Change densities to counts
    Count = tidyr::replace_na(Count, 0) # Replace NAs with 0 because NA signifies there are none of that species present
    )
```

# Quantifying biodiversity with real data

To explore these different diversity metrics and how they are calculated, we have subset data from the Turkey Lakes dataset across 4 years and 7 unique species to look at how diversity changes over time.
...
As an example, we use a subset of the Turkey Lakes data with seven species from the dataset to make a simple rank-abundance curve.
...
ggplot(aes(x=reorder(Species, -sum), y=sum, shape=Species, colour=Species))+
  ...
  theme(axis.text.x = element_text(face="italic")) # Use italics for species names
